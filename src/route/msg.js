...
module.exports = function registerPushHook({ app, bot }) {
  ...
  app.post(
    ...
    Service.handleError(async (req, res) => {
      ...
      const senderName = getSenderName(to, isRoom);

      const receiverNames = Array.isArray(to) ? to : [to];
      const msgReceivers = isRoom
        ? await Promise.all(
            receiverNames.map((name) => bot.Room.find({ topic: name }))
          )
        : await Promise.all(
            receiverNames.map((name) =>
              bot.Contact.find(
                Utils.equalTrueType(name, 'object') ? name : { name }
              )
            )
          );

      const sendStatuses = await Promise.all(
        msgReceivers.map((msgReceiver) => {
          if (msgReceiver) {
